...
begin
  // QUANDO CLICAR NO BOTÃO CINCO, EXIBE NO VISOR
...
begin
  //PERMITE QUE SEJÁ DIGITADO OS NÚMEROS
  ...
  if (Key = VK_NUMPAD0) then
    zero.Click;
  //PERMITE QYE SEJÁ DIGITADO OS SIMBOLOS
  if (Key = VK_ADD) then
    mais.Click;
...
begin
  ...
  case (funcao) of
    ...
    3:
      begin
        //EFETUA O CÁLCULO A MENOS QUE O USUÁRIO DESEJÁ DIVIDIR POR 0
        if (valor2 <> 0) then
        begin
          soma := valor1 / valor2;
          visor.Text := FloatToStr(soma);
        ...
        begin
          visor.Text := 'Erro: Divisão por Zero';
