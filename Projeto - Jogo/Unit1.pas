...
var
  ...
  atacando: Integer = 2; //1 Se o jogador está realizando um ataque, caso contrário 2
  desativar: boolean = false; //Para de passar pelo if se já passou uma vez
...
begin
  ...
  begin
    ...
    begin
      {Permite apenas uma animação ao mesmo tempo}
...
begin
  ...
  begin
    ...
    ShowMessage('Você Perdeu! Reincie o game para tentar novamente');
    desativar := true;
    jogadorVivo := false;
  end
  else
...
begin
  ...
  begin
    ShowMessage('Você Ganhou! Reincie o game se quiser jogar novamente');
    desativar := true;
    inimigoVivo := false;
  end
  else
